...
class ModelModuleSearchanise extends Model {
	...
	public function seGetProducts($p_id, $lang_code) {
		...
		if (empty($product['image'])) {
			$query_ = "SELECT image FROM `" . DB_PREFIX . "product_image` "
				. " WHERE product_id = " . $p_id;

			// [v1.5.1]
			if (version_compare(VERSION, '1.5.2', '<')) {
				$query_ .= " ORDER BY product_image_id ASC";
			// [/v1.5.1]
			// [v1.5.2] [v1.5.3] [v1.5.4] [v1.5.5] [v1.5.6]
			} else {
				$query_ .= " ORDER BY sort_order ASC";
			}
			// [/v1.5.2] [/v1.5.3] [/v1.5.4] [/v1.5.5] [/v1.5.6]

			$query_ .= " LIMIT 1";
			$additional_product_image = $this->db->query($query_);
